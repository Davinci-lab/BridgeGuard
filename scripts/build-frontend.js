...
<head>
  ...
  <title>CarthEdge BridgeGuard MVP</title>
  ...
  <main class="shell">
    <header class="hero">
      <div>
        ...
        <p class="subtitle">Explainable policy enforcement for simulated and connector-backed bridge risk.</p>
      </div>
    ...
    <section class="card connectors-card">
      <div class="card-head"><div><p class="eyebrow">Plug-and-play security layer</p><h2>Connectors</h2></div><button class="ghost-button" id="refresh-connectors">Refresh</button></div>
...
$('run').onclick=runSelected;$('run-all').onclick=runAll;$('create-connector').onclick=createConnector;$('refresh-connectors').onclick=loadConnectors;$('export').onclick=()=>{if(!lastDecision)return;const blob=new Blob([JSON.stringify(lastDecision,null,2)],{type:'application/json'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download='bridgeguard-decision.json';a.click();URL.revokeObjectURL(url)};$('attack').onchange=()=>renderIncident(selectedAttack());
